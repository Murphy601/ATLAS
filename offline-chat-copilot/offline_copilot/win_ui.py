...
def _process_live_chat(
    ...
    if result.save_logbook:
        say("[Copilot] Logbook DOM fill needs DevTools; skipping customer Other comment on desktop attach.")
    if result.fill_draft:
        filled = _fill_draft(hwnd, nodes, infos, result.fill_draft)
        if filled:
            say("[Copilot] Draft filled. Operator still sends — Send was not clicked.")
        else:
            say("[Copilot] Could not find the reply box. Paste option 1 yourself. Send was not clicked.")
    elif result.blocked:
        say(f"[Copilot] BLOCKED: {result.reason}. Draft box left empty.")


def _fill_draft(hwnd: int, nodes: list[Any], infos: list[dict], text: str) -> bool:
    if not text:
        return False
    ...
    if ctrl is not None:
        _uia_click(ctrl)
        time.sleep(0.12)
        if _uia_set_text(ctrl, text):
            return True
    _paste_into_focused(text)
    return True
...
def _paste_into_focused(text: str) -> None:
    """Clipboard paste. Never Enter — that would send the chat."""
